...
player = Player()
player.hideturtle()
turtle.register_shape("CharacterRightMario.gif")
player.shape("CharacterRightMario.gif")
player.pensize(100)
player.pu()
...
def winning_banner():
	global win,OBSTICLES_LIST,COINS_LIST
	win=True
	turtle.clear()
	turtle.goto(0,0)
	...
	time.sleep(3) 
	turtle.clear()
	for i in SPIKES_LIST:
		i.hideturtle()
		i.goto(1000,1000)
	for x in OBSTICLES_LIST:
		x.hideturtle()
		OBSTICLES_LIST=[]
		x.goto(1000,1000)
	for z in COINS_LIST:
...
def mario_main():
	...
	while stop == False:
		...
		while len(OBSTICLES_LIST) < OBSTICLES_MAX_NUMBER and win==False:
